...
//Valdez: data structures
//Pool: functional world map, walk, bounce obastcles, fight, beat game, load, save 
#include <vector>
#include <algorithm>
#include <iostream>
#include <fstream>
...
void fightMonster(vector<shared_ptr<Actor>> &party, int killsNeeded) {
	...
	std::sort(turnOrder.begin(), turnOrder.end(), speed_sort);
	...
	while (true) { // turn base loop
		...
		if (checkIfFightWon(party, enemies)) {
